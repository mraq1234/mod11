...
import { CREATE_LANE, CREATE_LANES, UPDATE_LANE, DELETE_LANE, MOVE_LANE } from './LaneActions';
...
export default function lanes(state = initialState, action) {
  switch (action.type) {
    ...
    case MOVE_NOTE:
      {
        const srcIdx = _.indexOf(state[action.sourceLaneId].notes, action.sourceNoteId);
        const trgIdx = _.indexOf(state[action.targetLaneId].notes, action.targetNoteId);
        ...
        if (action.sourceLaneId !== action.targetLaneId) {
          ...
          return { ...state, [action.sourceLaneId]: newSourceLane, [action.targetLaneId]: newTargetLane };
        }
        newSourceLane.notes.splice(srcIdx, 1);
        newSourceLane.notes.splice(trgIdx, 0, action.sourceNoteId);
        return { ...state, [action.sourceLaneId]: newSourceLane };
      }
    case MOVE_LANE:
      {
        let returnState = {};
        _.forEach(state, (val, key) => {
          if (key === action.targetLaneId) {
            returnState = { ...returnState, [action.sourceLaneId]: state[action.sourceLaneId] };
          } else if (key === action.sourceLaneId) {
            returnState = { ...returnState, [action.targetLaneId]: state[action.targetLaneId] };
          } else {
            returnState = { ...returnState, [key]: val };
          }
        });
        return returnState;
      }
